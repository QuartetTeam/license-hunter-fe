const certificateCardData =
    [
        {
            'certificateName': '정보처리기사',
            'receiptDate': '2025-08-12',
            'testDate': '2025-09-15',
            'calenderCount': 342
        },
        {
            'certificateName': '화학분석기사',
            'receiptDate': '2025-03-24',
            'testDate': '2025-05-18',
            'calenderCount': 567
        },
        {
            'certificateName': '전기기사',
            'receiptDate': '2025-02-15',
            'testDate': '2025-04-20',
            'calenderCount': 123
        },
        {
            'certificateName': '산업안전기사',
            'receiptDate': '2025-05-30',
            'testDate': '2025-07-12',
            'calenderCount': 789
        },
        {
            'certificateName': '건축기사',
            'receiptDate': '2025-04-18',
            'testDate': '2025-06-22',
            'calenderCount': 234
        },
        {
            'certificateName': '토목기사',
            'receiptDate': '2025-07-05',
            'testDate': '2025-08-30',
            'calenderCount': 456
        },
        {
            'certificateName': '대기환경기사',
            'receiptDate': '2025-09-14',
            'testDate': '2025-10-25',
            'calenderCount': 678
        },
        {
            'certificateName': '수질환경기사',
            'receiptDate': '2025-06-28',
            'testDate': '2025-08-15',
            'calenderCount': 890
        },
        {
            'certificateName': '임상심리사',
            'receiptDate': '2025-03-10',
            'testDate': '2025-04-28',
            'calenderCount': 345
        },
        {
            'certificateName': '소방설비기사',
            'receiptDate': '2025-05-22',
            'testDate': '2025-07-08',
            'calenderCount': 567
        },
        {
            'certificateName': '전자기사',
            'receiptDate': '2025-08-03',
            'testDate': '2025-09-20',
            'calenderCount': 789
        },
        {
            'certificateName': '자동차정비기사',
            'receiptDate': '2025-04-15',
            'testDate': '2025-06-01',
            'calenderCount': 234
        },
        {
            'certificateName': '보건위생사',
            'receiptDate': '2025-07-20',
            'testDate': '2025-09-05',
            'calenderCount': 567
        },
        {
            'certificateName': '식품기사',
            'receiptDate': '2025-03-28',
            'testDate': '2025-05-12',
            'calenderCount': 890
        },
        {
            'certificateName': '가스기사',
            'receiptDate': '2025-06-15',
            'testDate': '2025-08-01',
            'calenderCount': 123
        },
        {
            'certificateName': '의공기사',
            'receiptDate': '2025-09-08',
            'testDate': '2025-10-22',
            'calenderCount': 456
        },
        {
            'certificateName': '화학분석기사',
            'receiptDate': '2025-05-12',
            'testDate': '2025-06-28',
            'calenderCount': 789
        },
        {
            'certificateName': '전기기사',
            'receiptDate': '2025-08-25',
            'testDate': '2025-10-10',
            'calenderCount': 234
        },
        {
            'certificateName': '정보처리기사',
            'receiptDate': '2025-04-30',
            'testDate': '2025-06-15',
            'calenderCount': 567
        },
        {
            'certificateName': '산업안전기사',
            'receiptDate': '2025-07-18',
            'testDate': '2025-09-02',
            'calenderCount': 890
        },
        {
            'certificateName': '건축기사',
            'receiptDate': '2025-03-05',
            'testDate': '2025-04-20',
            'calenderCount': 123
        },
        {
            'certificateName': '토목기사',
            'receiptDate': '2025-06-22',
            'testDate': '2025-08-08',
            'calenderCount': 456
        },
        {
            'certificateName': '대기환경기사',
            'receiptDate': '2025-09-01',
            'testDate': '2025-10-18',
            'calenderCount': 789
        },
        {
            'certificateName': '수질환경기사',
            'receiptDate': '2025-05-15',
            'testDate': '2025-07-01',
            'calenderCount': 234
        },
        {
            'certificateName': '임상심리사',
            'receiptDate': '2025-08-08',
            'testDate': '2025-09-25',
            'calenderCount': 567
        },
        {
            'certificateName': '소방설비기사',
            'receiptDate': '2025-04-25',
            'testDate': '2025-06-10',
            'calenderCount': 890
        },
        {
            'certificateName': '전자기사',
            'receiptDate': '2025-07-12',
            'testDate': '2025-08-28',
            'calenderCount': 123
        },
        {
            'certificateName': '자동차정비기사',
            'receiptDate': '2025-03-18',
            'testDate': '2025-05-05',
            'calenderCount': 456
        },
        {
            'certificateName': '보건위생사',
            'receiptDate': '2025-06-05',
            'testDate': '2025-07-22',
            'calenderCount': 789
        },
        {
            'certificateName': '식품기사',
            'receiptDate': '2025-09-20',
            'testDate': '2025-11-05',
            'calenderCount': 234
        },
        {
            'certificateName': '가스기사',
            'receiptDate': '2025-05-08',
            'testDate': '2025-06-25',
            'calenderCount': 567
        },
        {
            'certificateName': '의공기사',
            'receiptDate': '2025-08-15',
            'testDate': '2025-10-01',
            'calenderCount': 890
        },
        {
            'certificateName': '화학분석기사',
            'receiptDate': '2025-04-02',
            'testDate': '2025-05-20',
            'calenderCount': 123
        },
        {
            'certificateName': '전기기사',
            'receiptDate': '2025-07-25',
            'testDate': '2025-09-10',
            'calenderCount': 456
        },
        {
            'certificateName': '정보처리기사',
            'receiptDate': '2025-03-12',
            'testDate': '2025-04-28',
            'calenderCount': 789
        },
        {
            'certificateName': '산업안전기사',
            'receiptDate': '2025-06-28',
            'testDate': '2025-08-15',
            'calenderCount': 234
        },
        {
            'certificateName': '건축기사',
            'receiptDate': '2025-09-05',
            'testDate': '2025-10-22',
            'calenderCount': 567
        },
        {
            'certificateName': '토목기사',
            'receiptDate': '2025-05-20',
            'testDate': '2025-07-08',
            'calenderCount': 890
        },
        {
            'certificateName': '대기환경기사',
            'receiptDate': '2025-08-18',
            'testDate': '2025-10-05',
            'calenderCount': 123
        },
        {
            'certificateName': '수질환경기사',
            'receiptDate': '2025-04-05',
            'testDate': '2025-05-22',
            'calenderCount': 456
        },
        {
            'certificateName': '임상심리사',
            'receiptDate': '2025-07-15',
            'testDate': '2025-09-01',
            'calenderCount': 789
        },
        {
            'certificateName': '소방설비기사',
            'receiptDate': '2025-03-28',
            'testDate': '2025-05-15',
            'calenderCount': 234
        },
        {
            'certificateName': '전자기사',
            'receiptDate': '2025-06-12',
            'testDate': '2025-07-30',
            'calenderCount': 567
        },
        {
            'certificateName': '자동차정비기사',
            'receiptDate': '2025-09-25',
            'testDate': '2025-11-12',
            'calenderCount': 890
        },
        {
            'certificateName': '보건위생사',
            'receiptDate': '2025-05-02',
            'testDate': '2025-06-18',
            'calenderCount': 123
        },
        {
            'certificateName': '식품기사',
            'receiptDate': '2025-08-20',
            'testDate': '2025-10-08',
            'calenderCount': 456
        },
        {
            'certificateName': '가스기사',
            'receiptDate': '2025-04-15',
            'testDate': '2025-06-01',
            'calenderCount': 789
        },
        {
            'certificateName': '의공기사',
            'receiptDate': '2025-07-08',
            'testDate': '2025-08-25',
            'calenderCount': 234
        },
        {
            'certificateName': '화학분석기사',
            'receiptDate': '2025-03-22',
            'testDate': '2025-05-08',
            'calenderCount': 567
        },
        {
            'certificateName': '전기기사',
            'receiptDate': '2025-06-18',
            'testDate': '2025-08-05',
            'calenderCount': 890
        },
        {
            'certificateName': '정보처리기사',
            'receiptDate': '2025-09-12',
            'testDate': '2025-10-30',
            'calenderCount': 123
        },
        {
            'certificateName': '산업안전기사',
            'receiptDate': '2025-05-25',
            'testDate': '2025-07-12',
            'calenderCount': 456
        }
    ];

export default certificateCardData;